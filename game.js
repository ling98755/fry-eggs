...
let egg = 5000000000000000n,
  ...
  egg_ene = [0, 0, 0, 0, 0], egg_enen = [0, 0, 0, 0, 0], egg_enene =0,
  mm1 = 0, mm2 = 0, reduce_egg_maker = 10000n, egg_pmm = 0,
  egg_upfee = [
  0,
  10000n,
  2000000n,
...
setInterval(() => {
  ...
  ene_up();
  if (rotate_mile[3] == 1)
    rotate_egg += log(egg)*20n;
  rotate_miles();
  if (egg_boost > 0n && egg_size<18000n+rotate_egg*2000n)
  {
    if(rotate_mile[0])
      egg_size += egg_boost * 40n * (5n ** ctoB(egg_ene[1]));
    else
    egg_size += egg_boost ;
  }
  ept =egg_maker *(1n + 2n * egg_boost*ctoB(egg_ene[3])) *up_legg ** legg_maker *2n *((egg_size/size_up)+1n)*egg_upgrade()*(4n**ctoB(egg_ene[4]));
  egg += ept;
  ...
  u.innerText =
    "重置煎蛋数量以获得" + log(egg / 50n)*(3n**ctoB(egg_ene[0])) + "个旋转煎蛋";
...
function egg_pmm_re()
{
  if (Log(rotate_egg) > egg_pmm)
    egg_pmm = Log(rotate_egg);
}
function ene(u) {
  if (egg_enene != 6)
    egg_enene = u;
